    ...
    if query.data.startswith("file"):
     ...
     #   if not search:
      ...
      #      return
        ...
        btn = [[
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="help"),
            InlineKeyboardButton("ᴄᴏɴᴛᴀᴄᴛ", url="t.me/EternalsHelplineBot")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            cap = f"<b><blockquote>Tʜᴇ Rᴇꜱᴜʟᴛꜱ Fᴏʀ {search}\nʀᴇsᴜʟᴛ sʜᴏᴡ ɪɴ {remaining_seconds} sᴇᴄᴏɴᴅs</blockquote>\n<blockquote>⚠️ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏᴍᴀᴛɪᴄᴀʟʟʏ ᴅᴇʟᴇᴛᴇᴅ 🗑️</blockquote></b>"
            for file in files:
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    try:
        ...
        button = [[
            InlineKeyboardButton("ᴄʟɪᴄᴋ ʜᴇʀᴇ ɪɴᴅᴇx", url="https://t.me/KafkaX_Bot?start=req_LTEwMDIyMTc3NDQ5MDk")], [InlineKeyboardButton("ᴄʜᴇᴄᴋ sᴘᴇʟʟɪɴɢ ɪɴ ɢᴏᴏɢʟᴇ", url=f"https://www.google.com/search?q={reqst_gle}")
        ]]
        if NO_RESULTS_MSG:
